...
The number being passed into the constructor for the SparkMaxs is the ID number of the motor, which we set using Rev hardware client.
...
At the top of the `DrivetrainSubsystem` file, add four `public static final int`s, two for the two left motors, and two for the right motors, then pass those to the `SparkMax` constructors.
...
Next we need to setup the differentialdrive object that will actually drive the robot.
